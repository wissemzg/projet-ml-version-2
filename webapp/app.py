...
sentiment_analyzer = SentimentAnalyzer(api_key=OPENAI_API_KEY) if (
    SentimentAnalyzer and OPENAI_API_KEY) else None
...
try:
    ...
    print(
        f"  [OK] Built code index for {len(_data_cache['code_index']):,} stocks.", flush=True)
    ...
    print(
        f"  [OK] Built tail cache for {len(_tail_cache):,} stocks.", flush=True)
...
def _quick_forecast_from_tail_payload(tail_payload: dict, stock_code: str, horizon: int = 5) -> dict:
    ...
    vol_amt = float(np.std(rets)) * \
        last_close if len(rets) >= 5 else last_close * 0.01
    lower = []
    upper = []
    ...
    try:
        ...
        forecast_dates = pd.bdate_range(
            start=last_ts + pd.Timedelta(days=1), periods=horizon)
    ...
    try:
        if len(close) >= 10:
            ...
            cov_ic = np.average(
                (x_ic - xm_ic) * (np.array(close) - ym_ic), weights=w_ic)
            ...
            if sigma2_ic > 0:
                ll_ic = -n_ic / 2.0 * \
                    (np.log(2 * np.pi) + np.log(sigma2_ic) + 1.0)
...
def _quick_forecast_from_df(stock_df: pd.DataFrame, stock_code: str, horizon: int = 5) -> dict:
    ...
    tail_close = np.asarray(close.tail(
        min(60, len(close))).values, dtype=float)
    rets = np.diff(tail_close) / np.maximum(tail_close[:-1], 1e-9)
    vol_amt = float(np.std(rets)) * \
        last_close if len(rets) >= 5 else last_close * 0.01
    lower = []
    ...
    avg_vol = float(sdf['volume'].dropna().tail(20).mean()
                    ) if 'volume' in sdf.columns else 0.0

    # Run quick ADF stationarity test
    stationarity_result = {"name": "close_price", "skipped": True}
    try:
        close_arr = np.asarray(close.tail(
            min(100, len(close))).values, dtype=float)
        if len(close_arr) >= 20:
            from statsmodels.tsa.stattools import adfuller
            _adf_res = adfuller(close_arr, maxlag=5, autolag=None)
            adf_stat, adf_p = float(_adf_res[0]), float(_adf_res[1])
    ...
    try:
        ...
        if len(close_vals) >= 10:
            ...
            cov_ic = np.average(
                (x_ic - xm_ic) * (close_vals - ym_ic), weights=w_ic)
            var_ic = np.average((x_ic - xm_ic) ** 2, weights=w_ic)
            sl_ic = cov_ic / var_ic if var_ic > 0 else 0
            int_ic = ym_ic - sl_ic * xm_ic
...
def get_sentiment_for_stock(stock_name, stock_code=None):
        ...
        if code:
            ...
            if len(sdf) >= 5:
                ...
                chg = ((last['close'] - prev['close']) /
                       prev['close'] * 100) if prev['close'] > 0 else 0
                avg_vol = sdf['volume'].mean()
                vol_chg = ((last['volume'] - avg_vol) /
                           avg_vol * 100) if avg_vol > 0 else 0
                high_30 = sdf['high'].max()
                low_30 = sdf['low'].min()
                returns_5d = ((sdf['close'].iloc[-1] - sdf['close'].iloc[-5]) /
                              sdf['close'].iloc[-5] * 100) if len(sdf) >= 5 else 0
    ...
    if sentiment_analyzer is not None:
        try:
            ...
            label = 'Positif' if sent == 'positive' else (
                'Négatif' if sent == 'negative' else 'Neutre')
            out = {
    ...
    try:
        ...
        if code:
            ...
            if len(sdf) >= 5:
                ret_5 = (sdf['close'].iloc[-1] -
                         sdf['close'].iloc[-5]) / sdf['close'].iloc[-5]
                ret_20 = (sdf['close'].iloc[-1] -
                          sdf['close'].iloc[0]) / sdf['close'].iloc[0]
                vol_ratio = sdf['volume'].iloc[-1] / \
                    max(sdf['volume'].mean(), 1)
                score = float(np.clip(ret_5 * 3 + ret_20 * 2 +
                              (vol_ratio - 1) * 0.2, -1, 1))
                label = 'Positif' if score > 0.15 else (
                    'Négatif' if score < -0.15 else 'Neutre')
                out = {
                    'sentiment': label, 'score': round(score, 3),
                    'positive_pct': round(max(50 + score * 60, 0), 1),
                    'negative_pct': round(max(50 - score * 60, 0), 1),
...
def api_market_overview():
    ...
    try:
        ...
        merged = today.merge(
            yesterday[['code', 'close']], on='code', suffixes=('', '_prev'))
        merged['change_pct'] = (
            (merged['close'] - merged['close_prev']) / merged['close_prev'] * 100).round(2)

        gainers = merged.nlargest(10, 'change_pct')[
            ['code', 'stock', 'close', 'change_pct', 'volume']].to_dict('records')
        losers = merged.nsmallest(10, 'change_pct')[
            ['code', 'stock', 'close', 'change_pct', 'volume']].to_dict('records')
        ...
        prev_tunindex = round(
            float(ti.iloc[-2]), 2) if len(ti) > 1 else current_tunindex
        tunindex_change = round(
            ((current_tunindex / prev_tunindex) - 1) * 100, 2) if prev_tunindex else 0
        ...
        global_sent_score = float(np.mean([get_sentiment_for_stock(
            s)['score'] for s in today['stock'].unique()[:3]]))
...
def api_stock_detail(stock_code):
    try:
        ...
        if len(sdf) >= 5:
            ret_5 = (sdf['close'].iloc[-1] - sdf['close'].iloc[-5]
                     ) / sdf['close'].iloc[-5]
            ret_20 = (sdf['close'].iloc[-1] -
                      sdf['close'].iloc[0]) / sdf['close'].iloc[0]
        ...
        sentiment_label = 'Positif' if score > 0.15 else (
            'Négatif' if score < -0.15 else 'Neutre')
        sentiment = {
        ...
        if len(closes) >= 10:
            ...
            fc_vals = [round(float(max(floor, closes[-1] + trend * (i+1))), 3)
                       for i in range(5)]
        else:
            last_p = float(closes[-1])
            floor = max(0.001, last_p * 0.05)
        ...
        pct_change = ((fc_vals[-1] - float(closes[-1])) /
                      float(closes[-1]) * 100) if closes[-1] > 0 else 0
        if pct_change > 2:
        ...
        result = {
            ...
            'stock': {
                ...
                'change_pct': safe_float(last_raw.get('return_pct',
                                                      ((last_raw['close'] - stock_df.iloc[-2]
                                                       ['close']) / stock_df.iloc[-2]['close'] * 100)
...
def api_stock_analysis(stock_code):
    ...
    try:
        df = get_cached_data()
        stock_df = df[df['code'] == stock_code].sort_values(
            'date').tail(500).copy()
        if stock_df.empty:
        ...
        fc_vals = forecast.get('ensemble', {}).get(
...
def api_forecast(stock_code):
    ...
    try:
        ...
        fast = request.args.get('fast', '1').strip(
        ...
        if fast:
            ...
            if isinstance(tail_cache, dict) and stock_code in tail_cache:
                result = _quick_forecast_from_tail_payload(
                    tail_cache[stock_code], stock_code, horizon=horizon)
            ...
            result = _quick_forecast_from_df(
                stock_df, stock_code, horizon=horizon)
            return jsonify(json_safe({'success': True, **result}))
        else:
            df = get_cached_data()
            stock_df = _get_stock_df_by_code(df, stock_code, tail=500)
...
def api_drift_analysis(stock_code):
    ...
    try:
        ...
        backtest = drift_analyzer.backtest_forecast(
            stock_df, lambda d, h: forecaster.forecast(
                d, horizon=h, fast=True),
...
def api_drift_compare(stock_code):
    ...
    try:
        ...
        pct_errors = [round(abs(a - p) / a * 100, 2) if a !=
                      0 else 0 for a, p in zip(actual_l, pred_l)]
...
def api_agents_analyze(stock_code):
    ...
    try:
        df = get_cached_data()
        stock_df = df[df['code'] == stock_code].sort_values('date').copy()
        stock_name = stock_df['stock'].iloc[-1] if len(
            stock_df) > 0 else stock_code
        ...
        fc_values = forecast.get('ensemble', {}).get(
            'forecast', forecast.get('forecast', []))
        models = list(forecast.get('ensemble', {}).get(
            'model_weights', {}).keys())
        ...
        pct_change = ((fc_last - last_price) /
                      last_price * 100) if last_price else 0
        if pct_change > 2:
            action = 'ACHETER'
...
def api_create_portfolio():
    ...
    try:
        ...
        user = None
    capital = float(data.get('capital', getattr(
        user, 'total_capital', 5000) if user else 5000))
    profile = data.get('risk_profile', getattr(
        user, 'risk_tolerance', 'moderate') if user else 'moderate')
...
def api_my_portfolio():
    ...
    if pid:
        df = get_cached_data()
        latest_date = df['date'].max()
        prices = df[df['date'] == latest_date].set_index('code')[
            'close'].to_dict()
...
def api_portfolio_buy(pid):
    ...
    if not shares:
        shares = 10
    result = db_buy_stock(pid, data['stock_code'], data.get(
        'stock_name', data['stock_code']), price, shares)
    return jsonify(result)
...
def api_portfolio_sell(pid):
    ...
    data = request.json
    shares = int(data.get('shares', 0)) if data.get('shares') else None
    result = db_sell_stock(
        pid, data['stock_code'], float(data['price']), shares)
    return jsonify(result)
...
def api_portfolio_suggest():
    ...
    try:
        ...
        capital_default = getattr(
            user, 'total_capital', 5000) if user else 5000
        profile_default = getattr(
            user, 'risk_tolerance', 'moderate') if user else 'moderate'
        ...
        futures = {_executor.submit(
            _analyze_stock, row): row['code'] for _, row in top_stocks.iterrows()}
        stocks_data = []
        ...
        suggestion = engine.generate_portfolio_suggestion(
            stocks_data, capital=capital)

        # Apply RL corrections if user is logged in
        uid = session.get('user_id')
        if uid and suggestion.get('allocations'):
            rl = PortfolioRL(uid)
            suggestion['allocations'] = rl.adjust_allocations(
...
def api_stock_ohlc(stock_code):
    ...
    try:
        ...
        def _extract_indicator(col_name):
            mask = stock_df[col_name].notna() if col_name in stock_df.columns else pd.Series(
                False, index=stock_df.index)
        ...
        macd_mask = stock_df['macd'].notna(
        ) if 'macd' in stock_df.columns else pd.Series(False, index=stock_df.index)
...
def api_stock_sarima(stock_code):
    ...
    try:
        ...
        include_xgb = request.args.get(
            'xgb', '0').strip().lower() in ('1', 'true', 'yes')

        # Stationarity
        stationarity = forecaster.check_stationarity(
            close_series, 'close_price')
        vol_stationarity = forecaster.check_stationarity(
...
def api_chat():
    ...
    try:
        ...
        merged = today.merge(
            yesterday[['code', 'close']], on='code', suffixes=('', '_prev'))
        merged['chg'] = ((merged['close'] - merged['close_prev']
                          ) / merged['close_prev'] * 100).round(2)
        gainers = merged.nlargest(
            5, 'chg')[['stock', 'close', 'chg']].to_dict('records')
        losers = merged.nsmallest(
            5, 'chg')[['stock', 'close', 'chg']].to_dict('records')
    ...
    try:
        ...
        if uid:
            ...
            if pid:
                prices = {}
                try:
                    latest_date_pf = df['date'].max(
                    ) if 'df' in dir() else None
                    if latest_date_pf is not None:
                        prices = df[df['date'] == latest_date_pf].set_index('code')[
                            'close'].to_dict()
                except Exception:
                    pass
                pf = db_get_portfolio(pid, prices)
...
def api_onboarding():
    ...
    if 'sectors' in data:
        updates['preferred_sectors'] = data['sectors'] if isinstance(
            data['sectors'], list) else [data['sectors']]
    if 'goals' in data:
